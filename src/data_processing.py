...
class DataProcessing:
    ...
    def connect_to_db_read_data(self):
        try:
            ...
            self.df = pd.read_sql(
                'SELECT * FROM public."ESG"', engine
            )  # ESG table exists in the database and can be viewed in DBeaver
    ...
    def process_feature_engineer_data(self):
        try:
            self.df["scope1_intensity"] = (
                self.df["scope1_emissions_mt_co2e"] / self.df["revenue_usd_bn"]
            )
            self.df["scope2_intensity"] = (
                self.df["scope2_emissions_mt_co2e"] / self.df["revenue_usd_bn"]
            )
            self.df["scope3_intensity"] = (
                self.df["scope3_emissions_mt_co2e"] / self.df["revenue_usd_bn"]
            )

            self.df["total_emissions"] = (
                self.df["scope1_emissions_mt_co2e"]
                + self.df["scope2_emissions_mt_co2e"]
                + self.df["scope3_emissions_mt_co2e"]
            )
            ...
            self.df["esg_emission_gap"] = (
                self.df["esg_score_0_100"] / self.df["carbon_intensity_tco2e_per_musd"]
            )
    ...
    def transform_encode(self):
        try:
            ...
            self.X_train[skewed_features] = pt.fit_transform(
                self.X_train[skewed_features]
            )
            ...
            self.X_train = pd.concat(
                [self.X_train.drop(columns=categorical_cols), encoded_df_train], axis=1
            )
            self.X_test = pd.concat(
                [self.X_test.drop(columns=categorical_cols), encoded_df_test], axis=1
            )

            # For future prediciton save
            os.makedirs(PROCESSED, exist_ok=True)

            os.makedirs(os.path.dirname(POWER_TRANSFORMER_PATH), exist_ok=True)
