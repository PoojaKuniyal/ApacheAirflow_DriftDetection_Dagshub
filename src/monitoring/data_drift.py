...
class DataDriftMonitor:
    ...
    def load_reference_data(self, path):
        try:
            ...
            logger.info(
                f"Successfully loaded the reference data.. with {self.reference_df.shape} rows and columns"
            )
    ...
    def load_current_data(self, path):
        try:
            ...
            logger.info(
                f"Successfully loaded the current data.. with {self.current_df.shape} rows and columns"
            )
        except Exception as e:
            raise CustomException(str(e), e)
    ...
    def preprocess(self):
        try:
            ...
            cur["scope1_intensity"] = (
                cur["scope1_emissions_mt_co2e"] / cur["revenue_usd_bn"]
            )
            cur["scope2_intensity"] = (
                cur["scope2_emissions_mt_co2e"] / cur["revenue_usd_bn"]
            )
            cur["scope3_intensity"] = (
                cur["scope3_emissions_mt_co2e"] / cur["revenue_usd_bn"]
            )

            cur["total_emissions"] = (
                cur["scope1_emissions_mt_co2e"]
                + cur["scope2_emissions_mt_co2e"]
                + cur["scope3_emissions_mt_co2e"]
            )
            ...
            cur["esg_emission_gap"] = (
                cur["esg_score_0_100"] / cur["carbon_intensity_tco2e_per_musd"]
            )
            ...
            cdp_map = {"D": 1, "C": 2, "B-": 3, "B": 4, "A-": 5, "A": 6}
            ...
            if missing_cols:
                raise ValueError(
                    f"Current data is missing required columns: {missing_cols}"
                )

            if extra_cols:
                logger.warning(
                    f"Dropping unexpected current-data columns: {extra_cols}"
                )

            cur = cur.reindex(
                columns=self.feature_names
            )  # Align column order with the training dataset or reference dataset

            os.makedirs(os.path.dirname(CURRENT_PROCESSED_DATA), exist_ok=True)
    ...
    def detect_get_drift_summary(self):
        try:
            ...
            for col in X_ref_df.columns:
                if X_ref_df[col].nunique() <= 1 or X_cur_df[col].nunique() <= 1:
            ...
            snapshot = report.run(reference_data=X_ref_df, current_data=X_cur_df)
            ...
            categorical_col = [
                ...
                if col not in numerical_cols and col not in constant_cols
            ]
            ...
            drift_score = len(drifted_features) / len(
                X_ref_df.columns
            )  # 0.2 means 20% of features show drift.
